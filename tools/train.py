...
from pcdet.models import build_network, model_fn_decorator, model_st_decorator
from pcdet.utils import common_utils
from pcdet.utils.optim import Lookahead, ModelSmoothing
...
def parse_config():
    ...
    parser.add_argument("--fix_random_seed", action="store_false", help="")
    ...
    parser.add_argument(
        "--mem_occumpy", type=float, default=0, help="max waiting minutes"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--using_lookahead", action="store_true", default=False, help=""
    )
    parser.add_argument(
        "--model_smoothing", action="store_true", default=False, help=""
    )
...
def occumpy_mem(cuda_device, fraction):
    total, used = check_mem(cuda_device)
    total = int(total)
    used = int(used)
    max_mem = int(total * 0.9)
    block_mem = int(max_mem * fraction * 0.95)
...
def main():
    ...
    if args.mem_occumpy > 0:
        occumpy_mem(cuda_device, args.mem_occumpy)
    ...
    train_model(
        ...
        if not cfg.MODEL.get("SELF_TRAINING", False)
        else model_st_decorator(),
        ...
        merge_all_iters_to_one_epoch=args.merge_all_iters_to_one_epoch,
        smooth_model=ModelSmoothing(model, args.smoothing_factor)
    ...
    args.start_epoch = max(args.epochs - 10, 0)  # Only evaluate the last 15 epochs
