...
def visualize_pts(
    ...
    if show_intensity:
        G = mlab.points3d(
            pts[:, 0], pts[:, 1], pts[:, 2], pts[:, 3], scale_factor=0.2, figure=fig
        )
    else:
        ...
        G.mlab_source.dataset.point_data.scalars = pts[:, 3]
...
def draw_scenes(
    ...
    if ref_rois3d is not None and not isinstance(ref_rois3d, np.ndarray):
        ref_rois3d = ref_rois3d.cpu().numpy()

    fig = visualize_pts(points, fig, show_intensity=False)
    # fig = draw_multi_grid_range(fig, bv_range=(25, -5, 55, 25), grid_size=2)
    if gt_boxes is not None:
        corners3d = boxes_to_corners_3d(gt_boxes)
        fig = draw_corners3d(corners3d, fig=fig, color=(0, 0, 0.8), max_num=100)

    if ref_rois3d is not None:
        corners3d = boxes_to_corners_3d(ref_rois3d)
        fig = draw_corners3d(
            corners3d, fig=fig, color=(1, 0, 0), max_num=100, cls=ref_scores
        )
    ...
    # mlab.view(172.85772109735075, 72.0359922746209, 85.95041322314063, np.array([31.32800322, -4.25069213,  3.53473006]))
    # mlab.view(153.97461764518573, 64.37090303332819, 12.775981251661035, np.array([39.69959501,  5.29230279, -1.14392463]))
    # mlab.view(-119.9246822520925, 39.15785598052371, 72.13604466297808, np.array([35.04497566, -0.31855001,  1.95133954]))
    mlab.view(
        -133.96914862035,
        25.17220611017552,
        105.61438299106852,
        np.array([35.59960273, 1.18731788, 2.51333745]),
    )
