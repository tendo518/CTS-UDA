...
def parse_config():
    ...
    parser.add_argument("--save_preds", action="store_true", default=False, help="")
    parser.add_argument("--smooth_model", action="store_true", default=False, help="")
    parser.add_argument("--smooth_step", type=int, default=3, help="")
...
def smooth_model(ckpt_list, ckpt_dir):
    num_list = [re.findall("checkpoint_epoch_(.*).pth", x)[-1] for x in ckpt_list]
    ckpt_name = "checkpoint_epoch_%s.pth" % "_".join(num_list)
    os.makedirs(os.path.join(ckpt_dir, "smoothed_ckpt"), exist_ok=True)
    ckpt_dir = os.path.join(ckpt_dir, "smoothed_ckpt", ckpt_name)
    if not os.path.exists(ckpt_dir):
        ckpts = [torch.load(x, map_location="cpu") for x in ckpt_list]
        state_dict0 = ckpts[0]["model_state"]
        for k in state_dict0.keys():
            if state_dict0[k].dtype.is_floating_point:
                state_dict0[k] = sum(s["model_state"][k] for s in ckpts) / len(ckpts)
        ckpts[0]["model_state"] = state_dict0
        torch.save(ckpts[0], ckpt_dir)
    return ckpt_dir
...
def get_no_evaluated_ckpt(ckpt_dir, ckpt_record_file, args):
    ...
    for i, cur_ckpt in enumerate(ckpt_list):
        ...
        if (
            ...
            if hasattr(args, "smooth_model") and args.smooth_model:
                if len(ckpt_list[i : i + args.smooth_step]) < args.smooth_step:
                    return -1, None
                else:
                    return epoch_id, smooth_model(
                        ckpt_list[i : i + args.smooth_step], args.ckpt_dir
                    )
            else:
...
def repeat_eval_ckpt(
    ...
    first_eval = True
    os.makedirs(
        eval_output_dir / "all" / cfg.DATA_CONFIG.DATA_SPLIT["test"], exist_ok=True
    )
    while True:
        ...
        tb_dict, write_line = eval_utils.eval_one_epoch(
            cfg,
            model,
            test_loader,
            cur_epoch_id,
        ...
        write_line.insert(0, str(cur_epoch_id))

        with open(
            eval_output_dir / "all" / cfg.DATA_CONFIG.DATA_SPLIT["test"] / "res.csv",
            "a",
        ) as f:
...
def main():
    ...
    if not args.eval_all:
        ...
        eval_output_dir = (
            eval_output_dir
            / ("epoch_%s" % epoch_id)
            / (args.split or cfg.DATA_CONFIG.DATA_SPLIT["test"])
        )
    else:
