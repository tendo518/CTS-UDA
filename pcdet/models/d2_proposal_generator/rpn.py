...
from pcdet.layers import ShapeSpec, cat
from pcdet.structures import Boxes, ImageList, Instances, pairwise_iou

# from pcdet.utils.events import get_event_storage
from pcdet.utils.memory import retry_if_cuda_oom
from ..model_utils.anchor_generator import build_anchor_generator
from ..model_utils.box_regression import Box2BoxTransform
...
class RPN(nn.Module):
    ...
    def from_config(cls, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        ret = {
            ...
            "loss_weight": {
                "loss_rpn_cls": cfg.LOSS_WEIGHT,
                "loss_rpn_loc": cfg.BBOX_REG_LOSS_WEIGHT * cfg.LOSS_WEIGHT,
            },
            "anchor_boundary_thresh": cfg.BOUNDARY_THRESH,
            "box2box_transform": Box2BoxTransform(weights=cfg.BBOX_REG_WEIGHTS),
            "box_reg_loss_type": cfg.BBOX_REG_LOSS_TYPE,
            "smooth_l1_beta": cfg.SMOOTH_L1_BETA,
            "pre_nms_topk": (cfg.PRE_NMS_TOPK_TRAIN, cfg.PRE_NMS_TOPK_TEST),
            "post_nms_topk": (cfg.POST_NMS_TOPK_TRAIN, cfg.POST_NMS_TOPK_TEST),
            "anchor_generator": build_anchor_generator(
                cfg, [input_shape[f] for f in in_features]
            ),
            "anchor_matcher": Matcher(
                cfg.IOU_THRESHOLDS, cfg.IOU_LABELS, allow_low_quality_matches=True
            ),
    ...
    def label_and_sample_anchors(
        ...
        """
        Args:
            self_training_mode: whether training with self supervised label
        ...
        gt_labels = []
        matched_gt_boxes = []
        for image_size_i, gt_boxes_i, gt_classes_i in zip(
            ...
            if self_training_mode and len(gt_boxes_i) > 0:
                ignored_box = gt_classes_i[matched_idxs] == -1
                gt_labels_i[ignored_box] = -1
    ...
    def forward(
        ...
        """
        Args:
            self_training_mode: use self supervised label
        ...
        if self.training:
            ...
            gt_labels, gt_boxes = self.label_and_sample_anchors(
                anchors, gt_instances, self_training_mode
            )
            losses = self.losses(
