...
from pcdet.layers import Linear, ShapeSpec, batched_nms, cat, nonzero_tuple
from pcdet.models.model_utils.box_regression import Box2BoxTransform
...
def fast_rcnn_inference_single_image(
    ...
    # scores = scores[:, :-1]
    scores = scores[:, 1:]  # bg labeled with 0
...
class FastRCNNOutputs:
    ...
    def _log_accuracy(self):
        ...
        # storage = get_event_storage()
        if num_instances > 0:
            self.tb_dict["fast_rcnn/cls_accuracy"] = num_accurate / num_instances
            if num_fg > 0:
                self.tb_dict["fast_rcnn/fg_cls_accuracy"] = fg_num_accurate / num_fg
                self.tb_dict["fast_rcnn/false_negative"] = num_false_negative / num_fg
    ...
    def box_reg_loss(self):
        ...
        fg_inds = nonzero_tuple(self.gt_classes >= 1)[0]
        ...
        if cls_agnostic_bbox_reg:
            ...
            # gt_classes start from 1
            gt_class_cols = box_dim * (fg_gt_classes[:, None] - 1) + torch.arange(
...
class FastRCNNOutputLayers(nn.Module):
    ...
    def from_config(cls, cfg, input_shape):
        return {
            ...
            # fmt: off
            "num_classes"           : cfg.IMAGE_HEAD_NUM_CLASSES,
            "cls_agnostic_bbox_reg" : cfg.ROI_BOX_HEAD.CLS_AGNOSTIC_BBOX_REG,
            "smooth_l1_beta"        : cfg.ROI_BOX_HEAD.SMOOTH_L1_BETA,
            "test_score_thresh"     : cfg.SCORE_THRESH_TEST,
            "test_nms_thresh"       : cfg.NMS_THRESH_TEST,
            "test_topk_per_image"   : cfg.DETECTIONS_PER_IMAGE_TEST,
            "box_reg_loss_type"     : cfg.ROI_BOX_HEAD.BBOX_REG_LOSS_TYPE,
            "loss_weight"           : {"loss_box_reg": cfg.ROI_BOX_HEAD.BBOX_REG_LOSS_WEIGHT},
            # fmt: on
        }
    ...
    def predict_boxes_for_gt_classes(self, predictions, proposals):
        ...
        if K > 1:
            ...
            gt_classes = (gt_classes - 1).clamp_(0, K - 1)
