...
from pcdet.layers import Conv2d, ConvTranspose2d, ShapeSpec, cat, get_norm
from pcdet.structures import Instances
...
def mask_rcnn_inference(
    ...
    if cls_agnostic_mask:
        ...
        class_pred = cat([i.pred_classes for i in pred_instances]) - 1
...
class MaskRCNNConvUpsampleHead(BaseMaskRCNNHead, nn.Sequential):
    ...
    def from_config(cls, cfg, input_shape):
        ...
        conv_dim = cfg.ROI_MASK_HEAD.CONV_DIM
        num_conv = cfg.ROI_MASK_HEAD.NUM_CONV
        ret.update(
            conv_dims=[conv_dim] * (num_conv + 1),  # +1 for ConvTranspose
            conv_norm=cfg.ROI_MASK_HEAD.NORM,
            input_shape=input_shape,
        )
        if cfg.ROI_MASK_HEAD.CLS_AGNOSTIC_MASK:
            ret["num_classes"] = 1
        else:
            ret["num_classes"] = cfg.IMAGE_HEAD_NUM_CLASSES
        return ret
...
def build_mask_head(cfg, input_shape):
    """
    Build a mask head defined by `cfg.ROI_MASK_HEAD.NAME`.
    """
    name = cfg.ROI_MASK_HEAD.NAME
