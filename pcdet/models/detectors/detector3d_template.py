...
class Detector3DTemplate(nn.Module):
    ...
    # def build_image_rpn(self, model_info_dict):
    #     if self.model_cfg.get('IMAGE_RPN', None) is None:
    #         return None, model_info_dict
    #     module = image_proposal_generator.__all__[self.model_cfg.IMAGE_RPN](
    #         self.model_cfg.IMAGE_RPN,
    #         model_info_dict['backbone_output_shape']
    #     )
    #     return module, model_info_dict

    # TODO image neck, image roi
    def build_backbone_image(self, model_info_dict):
        if self.model_cfg.get("BACKBONE_IMAGE", None) is None:
            return None, model_info_dict
        module = d2_backbone.__all__[self.model_cfg.BACKBONE_IMAGE.NAME](
            model_cfg=self.model_cfg.BACKBONE_IMAGE
        )
        if self.model_cfg.BACKBONE_IMAGE.get("FREEZE", False):
            for param in module.parameters():
                param.requires_grad = False

        model_info_dict["module_list"].append(module)
        model_info_dict["backbone_output_shape"] = module.output_shape()
        model_info_dict["size_divisibility"] = module.size_divisibility

        return module, model_info_dict

    def build_da(self, model_info_dict):
        if self.model_cfg.get("DOMAIN_ADAPTER", None) is None:
            return None, model_info_dict
        module = domain_adapter.__all__[self.model_cfg.DOMAIN_ADAPTER.NAME](
            model_cfg=self.model_cfg.DOMAIN_ADAPTER
        )
    ...
    def build_roi_head(self, model_info_dict):
        if self.model_cfg.get("ROI_HEAD", None) is None:
            return None, model_info_dict
        image_head_num_classes = self.num_class
        if self.model_cfg.get("IMAGE_BRANCH_WEIGHT", "") != "":
            if self.model_cfg.IMAGE_PRETRAINED_DATASET == "cityscapes":
                image_head_num_classes = 8
                self.cityscape_map = {}  # TODO
            else:
                raise NotImplementedError
        kwargs = (
            {
                "input_shape": model_info_dict.get("backbone_output_shape"),
                "real_classes": self.num_class,
                "image_head_num_classes": image_head_num_classes,
                "size_divisibility": model_info_dict.get("size_divisibility"),
            }
            if self.model_cfg.ROI_HEAD.NAME == "FusionRCNNHead"
            else {}
        )
    ...
    def load_params_from_file(self, filename, logger, to_cpu=False):
        ...
        for key, val in model_state_disk.items():
            if (
                key in state_dict
                and state_dict[key].shape == model_state_disk[key].shape
            ):
    ...
    @staticmethod
    def generate_au_record(box_preds, au_preds, gt_boxes, iou_thresh, alpha):
        # alpha is some value like 5%
        cur_gt = gt_boxes
        k = cur_gt.__len__() - 1
        while k > 0 and cur_gt[k].sum() == 0:
            k -= 1
        ...
        attr_cnt = au_preds.shape[-1]
        uncer_type = {8: "corner", 7: "box", 24: "corner_xyz"}[attr_cnt]
        if cur_gt.shape[0] > 0 and box_preds.shape[0] > 0:
            ...
            au_preds = au_preds[max_iou > iou_thresh]
            gt_index = gt_index[max_iou > iou_thresh]
            gt_boxes = gt_boxes[gt_index][:, 0:7]
            if gt_boxes.shape[0] > 0 and box_preds.shape[0] > 0:
                if uncer_type == "corner" or uncer_type == "corner_xyz":
                    box_preds = (
                        box_utils.boxes_to_corners_3d(box_preds)
                        .permute([0, 2, 1])
                        .contiguous()
                    )
                    gt_boxes = (
                        box_utils.boxes_to_corners_3d(gt_boxes)
                        .permute([0, 2, 1])
                        .contiguous()
                    )
                    if uncer_type == "corner":
                        au_preds = (
                            au_preds.unsqueeze(-2).expand_as(box_preds).contiguous()
                        )
                    else:
                        au_preds = torch.reshape(au_preds, box_preds.shape).contiguous()
                elif uncer_type == "box":
                    pass
                else:
                    raise NotImplementedError

                mean = box_preds.view(-1, attr_cnt)
                sigma = au_preds.sqrt().view(-1, attr_cnt)
                gt_boxes = gt_boxes.view(-1, attr_cnt)
                ...
                # if valid_mask.any():
                picp = valid_mask.mean(dim=0)

                # valid_label_mask = valid_mask.sum(dim=0) > 0
                valid_attr_cnt = torch.clamp_min(valid_mask.sum(dim=0), 1.0)
                # valid_cnt = torch.where(valid_label_mask, valid_mask.sum(dim=0), )
                mpiw = (2 * interval * valid_mask).sum(dim=0) / valid_attr_cnt
                # torch.where(valid_mask == 0, 1, valid_mask.sum(dim=0))
                mpiw = (2 * interval).mean(dim=0)
                # if for some attr valid is 0
                # then mpiw should be 0
                mpiw = torch.nan_to_num(mpiw, nan=0)
                soften = 1
                soft_mpiw = torch.sigmoid(mpiw * soften)
                return {
                    "uncer_picp": picp,
                    "uncer_mpiw": mpiw,
