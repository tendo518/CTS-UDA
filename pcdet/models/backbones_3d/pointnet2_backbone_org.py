...
class PointNet2MSGOrg(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        for i in range(len(self.SA_modules)):
            li_xyz, li_features, indices = self.SA_modules[i](l_xyz[i], l_features[i])
            l_xyz.append(li_xyz)
            l_features.append(li_features)
            feature_list.append((indices, li_features))
        ...
        for i in range(-1, -(len(self.FP_modules) + 1), -1):
            ...
            feature_list.append(
                (feature_list[len(self.SA_modules) + i][0], l_features[i - 1])
            )

        point_features = l_features[0].permute(0, 2, 1).contiguous()  # (B, N, C)
        batch_dict["point_features_all"] = feature_list
...
class PointNet2BackboneOrg(nn.Module):
    """
