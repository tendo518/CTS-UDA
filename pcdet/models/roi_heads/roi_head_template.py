...
class RoIHeadTemplate(nn.Module):
    ...
    def proposal_layer(self, batch_dict, nms_config):
        ...
        Returns:
            batch_dict:
                rois: (B, num_rois, 7+C) , B -> batch size
                roi_scores: (B, num_rois), maximum class score
                roi_labels: (B, num_rois), hard pred label
    ...
    def get_box_reg_layer_loss(self, forward_ret_dict):
        ...
        rcnn_batch_size = gt_boxes3d_ct.view(-1, code_size).shape[0]  # 一般是 512

        fg_mask = reg_valid_mask > 0
        fg_sum = fg_mask.long().sum().item()

        tb_dict = {}
        # add tensorboard attr
