...
class ProposalTargetLayer(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        Returns:
            batch_dict:
                rois: (B, M, 7 + C), samples 128 rois from 512 input roi, where
                128 rois consist of 64 bg rois and 64 fg rois
                gt_of_rois: (B, M, 7 + C), decode ground truth of 128 ious
                gt_iou_of_rois: (B, M), iou loss ?
                roi_scores: (B, M), 128 ious score, maximum class score
                roi_labels: (B, M), 128 ious hard pred label
                reg_valid_mask: (B, M), 0,1 mask, the coefficient of reg loss
                rcnn_cls_labels: (B, M), 0,1 mask, 128 roi fg/bg label
        """
        self_training_mode = batch_dict.get("self_training_mode", False)
        ...
        ignored_rois = (
            (batch_gt_of_rois[:, :, -1] == -1) if self_training_mode else None
        )
        if self_training_mode:
            reg_valid_mask[ignored_rois] = 0
        ...
        if self.roi_sampler_cfg.CLS_SCORE_TYPE == "cls":
            ...
            if self_training_mode:
                ignore_mask = ignore_mask | ignored_rois
    ...
    def sample_rois_for_rcnn(self, batch_dict):
        ...
        for index in range(batch_size):
            ...
            if self.roi_sampler_cfg.get(
                "SAMPLE_ROI_BY_EACH_CLASS", False
            ) and not batch_dict.get("self_training_mode", False):
    ...
    def get_max_iou_with_same_class(rois, roi_labels, gt_boxes, gt_labels):
        ...
        for k in range(min(gt_labels.min().item(), 1), gt_labels.max().item() + 1):
