...
class PointHeadTemplate(nn.Module):
    ...
    def build_losses(self, losses_cfg):
        ...
        elif reg_loss_type == "AULoss":
            self.reg_loss_func = loss_utils.AULoss(
    ...
    def assign_stack_targets(
        ...
        for k in range(batch_size):
            bs_mask = bs_idx == k
            points_single = points[bs_mask][:, 1:4]  # point in single batch
        ...
        targets_dict = {
            "point_cls_labels": point_cls_labels,  # fg or bg
            "point_box_labels": point_box_labels,  # box for each point, (rxryrz, log(hwl), cos(theta), sin(theta))
            "point_part_labels": point_part_labels,
        }  # groundtruth from gt box
        return targets_dict
