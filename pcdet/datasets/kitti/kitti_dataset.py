...
class KittiDataset(DatasetTemplate):
    def __init__(
        ...
        self.dataset_name = os.path.split(self.root_path)[-1]
        self.split = (
            split if split is not None else self.dataset_cfg.DATA_SPLIT[self.mode]
        )
    ...
    def include_kitti_data(self, mode):
        if self.logger is not None:
            self.logger.info("Loading %s dataset" % self.dataset_name)
        kitti_infos = []
        ...
        if self.logger is not None:
            self.logger.info(
                "Total samples for %s dataset: %d"
                % (self.dataset_name, len(kitti_infos))
            )
    ...
    def __getitem__(self, index):
        ...
        if "annos" in info:
            ...
            if self.dataset_cfg.FOV_POINTS_ONLY:
                # NOTE why add this?
                box_rect = calib.lidar_to_rect(gt_boxes_lidar[:, 0:3])
                box_fov_flag = self.get_fov_flag(box_rect, img_shape, calib)
                gt_boxes_lidar = gt_boxes_lidar[box_fov_flag]
                gt_names = gt_names[box_fov_flag]
...
def create_kitti_infos(dataset_cfg, class_names, data_path, save_path, workers=8):
    ...
    #     num_workers=workers, has_label=True, count_inside_pts=False
    # )
