...
class DataProcessor(object):
    ...
    def mask_points_and_boxes_outside_range2(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.mask_points_and_boxes_outside_range2, config=config)

        mask = common_utils.mask_points_by_range2(
            data_dict["points"], self.point_cloud_range
        )
        data_dict["points"] = data_dict["points"][mask]
        if (
        ...
        if num_points < len(points):
            ...
            if num_points > len(far_idxs_choice):
                ...
                choice = np.random.choice(
                    choice, num_points, replace=len(choice) * 2 < num_points
                )
            ...
            try:
                ...
                if num_points > len(points):
                    extra_choice = np.random.choice(
                        ...
                        replace=len(choice) * 2 < num_points,
                    )
...
#         fov_flag, pts_img = get_fov_flag(
#             pts_rect, data_dict["image_shape"], calib, True
#         )
...
#         if (
#             data_dict.get("gt_boxes", None) is not None and config.REMOVE_OUTSIDE_BOXES
#         ) and self.training:
#             mask = box_utils.mask_boxes_outside_range_numpy(
#                 data_dict["gt_boxes"],
#                 self.point_cloud_range,
...
#         affine_matrix = np.asarray([[r, 0.0, dw // 2], [0.0, r, dh // 2], [0, 0, 1]])
