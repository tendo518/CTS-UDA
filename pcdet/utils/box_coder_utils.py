...
class ResidualCoder(object):
    def __init__(self, code_size=7, encode_angle_by_sincos=False, **kwargs):
        ...
        if self.encode_angle_by_sincos:
            self.code_size += 1
        self.mean_size = torch.empty(0, 3).cuda()
        if "mean_size" in kwargs:
            self.mean_size = (
...
class PointResidualCoder(object):
    ...
    def encode_torch(self, gt_boxes, points, gt_classes=None):
        ...
        Returns:
            box_coding: (N, 8 + C)
        """
        # only fg point and its assigned gt box
        ...
        if self.use_mean_size:
            ...
            xt = xg - xa  # center point from fg point x residual
            yt = yg - ya  # center point from fg point y residual
            zt = zg - za  # center point from fg point z residual
            dxt = torch.log(dxg)  #
            dyt = torch.log(dyg)
            dzt = torch.log(dzg)

        cts = [g for g in cgs]
        # here cos and sin are used to encode the direction
        return torch.cat(
...
def box_uncertainty_decode(var, anchors, box_encodings):
    assert var.shape[-1] == box_encodings.shape[-1] == 7
    # print(mu.shape, anchors.shape, box_encodings.shape)
    # mu = torch.sqrt(mu)
    xa, ya, za, wa, la, ha, ra = torch.split(anchors, 1, dim=-1)
    xt, yt, zt, wt, lt, ht, rt = torch.split(box_encodings, 1, dim=-1)
    xc, yc, zc, wc, lc, hc, rc = torch.split(var, 1, dim=-1)
    ...
    lc = (torch.exp(lc) - 1) * torch.exp(2 * lt + lc) * (la**2)
    wc = (torch.exp(wc) - 1) * torch.exp(2 * wt + wc) * (wa**2)
    hc = (torch.exp(hc) - 1) * torch.exp(2 * ht + hc) * (ha**2)
